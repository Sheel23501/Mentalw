...
/**
 * Save a full video call transcript to Firestore.
 * @param {Object} data - { session_id, patient_id, doctor_id, transcript, duration_seconds }
 * @returns {Promise<string>} - The transcript document ID
 */
export const saveVideoTranscript = async (data) => {
  try {
    const ref = await addDoc(collection(db, 'video_transcripts'), {
      ...data,
      created_at: new Date().toISOString(),
    });
    console.log('🎥 Video transcript saved with ID:', ref.id);
    return ref.id;
  } catch (error) {
...
/**
 * Save an AI-generated summary for a video transcript.
 * @param {Object} data - { transcript_id, patient_id, summary_text, key_concerns, emotional_cues, action_items }
 * @returns {Promise<string>}
 */
export const saveTranscriptSummary = async (data) => {
  try {
    const ref = await addDoc(collection(db, 'transcript_summaries'), {
      ...data,
      created_at: new Date().toISOString(),
    });
    console.log('🧠 Transcript summary saved with ID:', ref.id);
    return ref.id;
  } catch (error) {
...
/**
 * Fetch all video transcripts for a patient, ordered by date descending.
 * @param {string} patientId
 * @returns {Promise<Array>}
 */
export const getVideoTranscriptsForPatient = async (patientId) => {
  ...
  try {
    const q = query(
      collection(db, 'video_transcripts'),
      where('patient_id', '==', patientId),
...
/**
 * Fetch AI summary for a specific transcript.
 * @param {string} transcriptId
 * @returns {Promise<Object|null>}
 */
export const getTranscriptSummary = async (transcriptId) => {
  if (!transcriptId) return null;
  try {
    const q = query(
      collection(db, 'transcript_summaries'),
...
/**
 * Fetch ALL historical GAD-7 results for a user (for timeline charts).
 * @param {string} userId
 * @returns {Promise<Array>} - Sorted oldest → newest
 */
export const getGad7HistoryForUser = async (userId) => {
  if (!userId) return [];
  try {
    const q = query(
      collection(db, 'gad7Results'),
      where('userId', '==', userId),
      orderBy('timestamp', 'asc')
    );
    const snap = await getDocs(q);
    ...
    console.error('Error fetching GAD-7 history:', error);
    return [];
  }
};

/**
 * Fetch emotion logs for a patient. Defaults to all-time; pass sinceHours to limit.
 * @param {string} patientId
 * @param {number|null} sinceHours - If provided, only returns logs from the last N hours
 * @returns {Promise<Array>}
 */
export const getEmotionLogsForPatient = async (patientId, sinceHours = null) => {
  if (!patientId) return [];
  try {
    let q;
    if (sinceHours) {
    ...
    return snap.docs
      .map(d => d.data())
      .filter(log => log.emotionLabel && log.emotionLabel !== 'neutral')
      .map(log => ({
        time: log.timestamp,
        emotion: log.emotionLabel,
...
/**
 * Fetch escalation history for a patient.
 * @param {string} patientId
 * @returns {Promise<Array>}
 */
export const getEscalationsForPatient = async (patientId) => {
  if (!patientId) return [];
  try {
    const q = query(
      collection(db, 'escalations'),
...
/**
 * Fetch ALL AI chat logs for a patient (not limited to 48hrs).
 * @param {string} patientId
 * @returns {Promise<Array>}
 */
export const getAllAIChatLogs = async (patientId) => {
  if (!patientId) return [];
  try {
    const q = query(
      collection(db, 'ai_chat_logs'),
    ...
    console.error('Error fetching all AI chat logs:', error);
    return [];
  }
};

/**
 * UNIFIED: Fetch the complete patient history for the doctor's Patient History Panel.
 * Aggregates all data sources in parallel for fast loading.
 * @param {string} patientId
 * @returns {Promise<Object>} - Full patient history object
 */
export const getFullPatientHistory = async (patientId) => {
  ...
  try {
    // Fetch user profile
    const userRef = doc(db, 'userProfiles', patientId);
    const userSnap = await getDoc(userRef);
    const profile = userSnap.exists() ? userSnap.data() : {};

    // Fetch all data sources in parallel
    const [gad7History, emotionLogs, aiChatLogs, sessionNotes, transcripts, escalations] = await Promise.all([
      getGad7HistoryForUser(patientId),
      getEmotionLogsForPatient(patientId),
      getAllAIChatLogs(patientId),
      getPatientSessionNotes(patientId),
      getVideoTranscriptsForPatient(patientId),
      getEscalationsForPatient(patientId),
    ]);

    // Compute GAD-7 trend
    let gad7Trend = 'stable';
    if (gad7History.length >= 2) {
      const last = gad7History[gad7History.length - 1].score;
      const prev = gad7History[gad7History.length - 2].score;
      if (last < prev) gad7Trend = 'improving';
      else if (last > prev) gad7Trend = 'worsening';
    }

    // Compute risk level
    let riskLevel = 'Low';
    if (profile.lastGad7Severity === 'Severe') {
      riskLevel = 'High Risk';
    } else if (escalations.some(e => e.status === 'pending')) {
      riskLevel = 'Needs Attention';
    } else if (emotionLogs.filter(e => ['anxious', 'fear', 'sad', 'stressed'].includes(e.emotion?.toLowerCase())).length > 10) {
      riskLevel = 'Needs Attention';
    }

    // Find last doctor session date (for "since last session" calculations)
    const lastSessionDate = sessionNotes.length > 0 ? sessionNotes[0].createdAt : null;

    // Get data since last session
    const sinceLastSession = {
      aiChats: lastSessionDate
        ? aiChatLogs.filter(log => log.timestamp > lastSessionDate)
        : aiChatLogs,
      emotions: lastSessionDate
        ? emotionLogs.filter(log => log.time > lastSessionDate)
        : emotionLogs,
      escalations: lastSessionDate
        ? escalations.filter(e => {
            const created = e.createdAt?.toDate ? e.createdAt.toDate().toISOString() : e.createdAt;
            return created > lastSessionDate;
          })
        : escalations,
    };
    ...
    return {
      profile,
      gad7: {
        current_score: profile.lastGad7Score ?? null,
        current_severity: profile.lastGad7Severity ?? 'Not Taken',
        history: gad7History,
        trend: gad7Trend,
      },
